    ...
    def _create_setting_widgets(self, filter: str) -> list[SettingWidget]:
        ...
        for title, setting_properties in self.settings.items():
            for name, setting_property in setting_properties.items():
                if not (filter in title or filter in name or filter in setting_property["description"]):
                    continue
...
def test_setting_widget():
    ...
    app = create_qt_app()
    app.setStyleSheet(
        app.styleSheet() + "*{font-size: 17px; font-family: Consolas;}" "SettingWidget::hover{background: gray;}"
    )
